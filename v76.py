...
def get_data(symbol, tf):
    try:
        ...
        elif 'h' in tf or 'H' in tf: fetch_tf, period = '1h', '730d'
        ...
        rule_map = {'2h':'2H', '4h':'4H', '8h':'8H', '12h':'12H', '3d':'3D', '1w':'W-MON', '3m':'3ME'}
        if tf in rule_map:
...
def main():
    st.markdown("<h1 style='text-align: left; color: #00ffcc;'>🏆 Master Trade Dashboard</h1>", unsafe_allow_html=True)
    now_vn = datetime.now(VN_TZ).strftime('%H:%M:%S - %d/%m/%Y')
    st.write(f"🕒 Cập nhật thực: {now_vn}")
    ...
    for asset in LIST_ASSETS:
        ...
        if data_rows:
            ...
            # DÙNG .style.map() THAY CHO .applymap() ĐỂ SỬA LỖI ATTRIBUTEERROR
