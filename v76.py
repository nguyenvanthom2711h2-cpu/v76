...
st.set_page_config(page_title="Master Trade v171", layout="wide")

# ==========================================
# 2. HÀM LẤY GIÁ LIVE (Fix lỗi 404 & Cache)
# ==========================================
def get_live_price_v171(symbol):
    if "BTC" in symbol:
        try:
            # Lấy từ Binance API - Không bao giờ bị 404
            url = f"https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT&_t={random.random()}"
            res = requests.get(url, timeout=3)
            return float(res.json()['price'])
        except: pass
    
    try:
        # Lấy từ Yahoo nhưng dùng Ticker và xóa cache
        t = yf.Ticker(symbol)
...
def fetch_history_v171(symbol, tf):
    try:
        # Thêm proxy/headers giả lập hoặc xóa cache
        p = '5d' if 'm' in tf else ('730d' if 'h' in tf else 'max')
        f_tf = '1h' if ('h' in tf and tf != '1h') else ('1d' if tf == '3d' else tf)
        
        # Tải dữ liệu (Yahoo thường chặn download hơn là history)
...
def main():
    st.markdown("<h2 style='text-align: center; color: #00ffcc;'>🚀 Master Trade Dashboard v171</h2>", unsafe_allow_html=True)
    
    # Sidebar control
    if st.sidebar.button("♻️ RESET CACHE & RELOAD"):
        st.cache_data.clear()
        st.rerun()
    ...
    for i, asset in enumerate(LIST_ASSETS):
        with tabs[i]:
            ...
            with st.spinner(f"Đang đồng bộ {asset['name']}..."):
                # Lấy giá Live trước (Phải nhảy số)
                live_p = get_live_price_v171(asset['symbol'])
                
                rows = []
                for tf in TIMEFRAMES:
                    if asset['symbol'] == "^VNINDEX" and ('m' in tf or 'h' in tf): continue
                    
                    df = fetch_history_v171(asset['symbol'], tf)
                    
                    # Ngay cả khi Yahoo lỗi nến, vẫn hiện giá Live
                    ...
                    if df is not None:
                        ...
                        # Nếu Yahoo block hoàn toàn, hiện giá Live và báo lỗi chỉ báo
                        rows.append({
                            "Khung": tf.upper(), "Xu hướng": "Đang tải lại...", "RSI 9/45": "Lỗi 404",
